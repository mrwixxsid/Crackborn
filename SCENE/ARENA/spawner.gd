...
@export var enemy_collection: Array[UnitStats]   ## Every time we complete a new wave, we need to update our enemies, damage and also increse health
...
var spawnd_enemies: Array[Enemy] = []
...
func set_spawn_timer():
	match current_wave_data.spawn_type:
		...
		WaveData.SpawnType.RANDOM: 
			var min_time = current_wave_data.min_spawn_time
			var max_time = current_wave_data.max_spawn_time
...
func spawn_enemy():
	...
	if enemy_scene:
		...
		spawnd_enemies.append(enemy_instance)
		
	set_spawn_timer()


func update_enemies_new_wave():
...
func clear_enemies():
	if spawnd_enemies.size() > 0:
		 ## Spawn Enemies array
		for enemy: Enemy in spawnd_enemies:
			if is_instance_valid(enemy):
				enemy.distry_enemy()
				
	spawnd_enemies.clear()
...
func _on_spawn_timer_timeout() -> void:
	## If we dont have current wave data, we cant spawn enemy
	## If our wave is completed, we cant spawn enemies 
